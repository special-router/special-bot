...
[[ $(docker inspect -f '{{.State.Running}}' remnawave-node) == true ]]
listener=$(ss -lntpH 'sport = :8443')
[[ $(wc -l <<<"$listener") -eq 1 ]]
grep -q 'rw-core' <<<"$listener"
echo 'nl_tcp_tuning=verified qdisc=fq congestion_control=bbr listeners_8443=1 data_plane=remnawave'
REMOTE
    ;;
  apply)
