...
CONFIG_EDGE_HOST=${SPECIAL_CONFIG_EDGE_HOST:-158.160.187.90}
CONFIG_EDGE_SSH_USER=${SPECIAL_CONFIG_EDGE_SSH_USER:-admin-user}
...
remote() {
  ...
  [[ $host == "$CONFIG_EDGE_HOST" ]] && user=$CONFIG_EDGE_SSH_USER
...
swap_bytes=$(remote "$BOT_HOST" 'swapon --show --noheadings --bytes | awk "{total += \$3} END {print total + 0}"')
...
firewall_rules=$(remote "$BOT_HOST" 'iptables -S DOCKER-USER | grep -- "--dport 8001"')
[[ $(wc -l <<<"$firewall_rules") == 4 ]]
grep -q -- "-s $NL_HOST/32 .* -j ACCEPT" <<<"$firewall_rules"
grep -q -- "-s $CONFIG_EDGE_HOST/32 .* -j ACCEPT" <<<"$firewall_rules"
grep -q -- "-j DROP" <<<"$firewall_rules"
...
[[ $nl_status == 404 ]]
edge_status=$(remote "$CONFIG_EDGE_HOST" "curl -s --connect-timeout 5 --max-time 15 -H 'Host: special-wifi.link' http://$BOT_HOST:8001/sub/does-not-exist -o /dev/null -w '%{http_code}'")
[[ $edge_status == 404 ]]
...
assert report['paging_delivery_verified'] is False
...
printf 'commit=%s port=%s swap_bytes=%s nl_status=%s edge_status=%s\n' "$commit" "$port_binding" "$swap_bytes" "$nl_status" "$edge_status"
printf '%s\n' "$legacy" | tail -2
printf '%s\n' "$monitoring" | tail -4
