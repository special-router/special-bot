...
| `SPECIAL_MONITOR_CHECKOUT_ENABLED` | bool | `False` | `false` | Additionally schedules the checkout layer every 15 min: one `create_invoice_link` against the live provider token, plus the days-since-cash-in gap. Requires `SPECIAL_MONITOR_ENABLED`. Off means the task records nothing at all, so no stale green state is left behind. |
...
| `SPECIAL_MONITOR_CHECKOUT_AMOUNT` | int | `10000` | ? | Face value of the probe invoice, in kopecks. Nobody pays the link, but the provider validates the amount when issuing it — too small and the probe reports a broken checkout when only the probe is misconfigured. |
