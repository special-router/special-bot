...
`🇳🇱 Нидерланды` on the subscription hostname, then `🇳🇱 Нидерланды белые списки`
if `server.client_vpn_host` is set — both endpoints we operate, before any
fallback — then, only for allowlisted test users, internal canary lines and
external backup lines. `flow` is deliberately empty: forcing Vision
