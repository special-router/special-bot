...
# todo: чатгпт хуйню нагенерил, надо переделать
...
class BroadcastAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Настройки отправки', {'fields': ('scheduled_at',)}),
        (
            'Статистика',
            {
                'fields': (
        ...
        (
            ...
            {'fields': ('created_by', 'created_at', 'updated_at', 'sent_at'), 'classes': ('collapse',)},
        ),
    )
    ...
    def status_badge(self, obj):
        ...
        colors = {'draft': 'gray', 'sending': 'orange', 'sent': 'green', 'failed': 'red'}
        color = colors.get(obj.status, 'gray')
        return format_html(
            '<span style="background-color: {}; color: white; padding: 2px 8px; border-radius: 3px; font-size:'
            ' 11px;">{}</span>',
            color,
    ...
    def preview_message(self, obj):
        ...
        return format_html(
            '<div style="background-color: #f8f9fa; padding: 10px; border-radius: 5px; border-left: '
            '4px solid #007bff; max-width: 600px;">{}</div>',
            preview.replace('\n', '<br>'),
        )
    ...
    def send_broadcast(self, request, queryset):
        ...
        if sent_count > 0:
            self.message_user(request, f'Запущена отправка {sent_count} рассылок', level=messages.SUCCESS)
    ...
    def duplicate_broadcast(self, request, queryset):
        ...
        self.message_user(request, f'Создано {duplicated_count} копий рассылок', level=messages.SUCCESS)
