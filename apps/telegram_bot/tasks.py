...
# todo: криво работающая хуйня от чатагпт
    ...
    try:
        ...
        for user in users:
            try:
                ...
                asyncio.run(bot.send_message(chat_id=user.telegram_id, text=broadcast.message, parse_mode='HTML'))
                sent_count += 1
                time.sleep(1)
                ...
                if failed_count > total_users * 0.5:  # Более 50% ошибок
                    ...
                    broadcast.error_message = (
                        f'Слишком много ошибок ({failed_count}/{total_users}). Остановка рассылки.'
                    )
                    broadcast.sent_count = sent_count
                    broadcast.failed_count = failed_count
                    broadcast.save()
